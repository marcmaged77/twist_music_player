...
| Piece | What it is |
|---|---|
| `TwistMusicSwimlane` | Header and an endless peek carousel. `style: plain` (default) is the logo, title and a promo chip on the host background; `style: banner` is the brand gradient band (rounded top, oval bottom, a glow drifting on the right) with the logo, title, the subtitle as offer text and a download pill, with the cards overlapping the band. Both open the download link. Also an endless peek carousel of artwork cards (artist and title over the art, a "Now Playing" capsule on the active one). Loads the lane itself. `loadingBuilder`, `emptyBuilder`, `onContentAvailabilityChanged`, `backgroundColor`, `cardWidth`. |
