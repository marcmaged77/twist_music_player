...
| Piece | What it is |
|---|---|
| `TwistMusicSwimlane` | Dark band with the header and an endless peek carousel of artwork cards (artist and title over the art, a "Now Playing" capsule on the active one). Loads the lane itself. `loadingBuilder`, `emptyBuilder`, `onContentAvailabilityChanged`, `backgroundColor`, `cardWidth`. |
